...
Mobile uses pinned `@clerk/expo`, hosted auth, and Clerk's secure token cache backed by `expo-secure-store`. Configure `EXPO_PUBLIC_SPORTABASE_DEPLOYMENT` as `local`, `staging`, or `production` and set an explicit `EXPO_PUBLIC_SPORTABASE_API_URL`; missing values leave the app in an `unconfigured` state in which public UI can render but Sportabase requests fail before `fetch()`. Staging rejects the production Sportabase API origin. Web auth in staging additionally requires `EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY` with a Clerk test publishable key and the exact HTTPS canonical web origin in `EXPO_PUBLIC_SPORTABASE_WEB_URL`. Missing or invalid auth configuration preserves the explicit unavailable account state, and the privacy link is omitted when the canonical origin is absent or invalid. AsyncStorage contains non-secret device IDs, the one-time migration marker, the current notification registration ID, and cached appearance values only.
