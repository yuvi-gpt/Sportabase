...
Endpoints are attacker-controlled. Registration and dispatch resolve every IPv4/IPv6 address and reject the destination if any answer is loopback, private, link-local, reserved, or otherwise non-global. Dispatch uses a dedicated `requests.Session` with redirects disabled, so a push service cannot redirect delivery into another host or internal address. Dispatch-time resolution narrows DNS-rebinding exposure, but application DNS checks cannot cryptographically pin a connection to the checked address; production should also deny private/link-local metadata ranges at the platform egress firewall or proxy.
...
Automated tests mock all provider calls. Do not treat Web Push delivery as production-verified until this gate succeeds:
