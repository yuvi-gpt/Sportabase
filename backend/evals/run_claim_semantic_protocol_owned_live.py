...
from .claim_semantic_protocol_owned_live import (
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run the bounded #35H one-call live validation through the #35G protocol ownership boundary."
        )
    )
    parser.add_argument(
        "--live",
    ...
    parser.add_argument(
        ...
        help="Must be exactly 1 for #35H.",
    )
    parser.add_argument(
        "--describe",
        action="store_true",
        help="Print the one-call plan and capacity with zero provider calls.",
    )
    parser.add_argument(
        "--json-out",
        default="",
...
def _print_plan() -> None:
    print("Sportabase #35H one-call protocol-owned live validation")
    ...
    print("same #35E prompt as #35F: TRUE")
    print("parse path: #35G -> #35E -> #35D")
...
def main(argv=None) -> int:
    ...
    if int(args.max_calls) != EXACT_PROVIDER_CALLS:
        print("#35H requires --max-calls 1 exactly.", file=sys.stderr)
        return 2
    ...
    print()
    print("=== LIVE GEMINI PROTOCOL-OWNED CALL LOG ===")
    try:
        report = evaluate_live_protocol_owned_hard_negative(
            api_key=api_key,
            usage_db_path=usage_db_path,
            max_calls=EXACT_PROVIDER_CALLS,
            event_sink=_provider_event_logger,
        )
    except ClaimSemanticProtocolOwnedLiveError as error:
    ...
    protocol = row.get("protocol_ownership") or {}
    ...
    print()
    print("=== #35G PROTOCOL OWNERSHIP ===")
    print(
        "model supplied candidate version:",
        protocol.get("candidate_contract_version_supplied_by_model"),
    )
    print(
        "candidate version removed before validation:",
        protocol.get("candidate_contract_version_removed_before_validation"),
    )
    print(
        "validator assigned candidate version:",
        protocol.get("validator_assigned_candidate_contract_version", ""),
    )
    print("semantic fields rewritten:", protocol.get("semantic_fields_rewritten"))
    print("status rewritten:", protocol.get("status_rewritten"))
    print("reason rewritten:", protocol.get("reason_rewritten"))
    ...
    if row.get("candidate") is not None:
        ...
        print("candidate_version:", candidate.get("version"))
    ...
    print("prompt_digest:", report["prompt_digest"])
    ...
    # Quality failure remains a measured result, not an execution failure.
    return 0
