...
def selected_cases(case_ids: Sequence[str]) -> list[Dict[str, Any]]:
    ...
    if not normalized or any(not value for value in normalized):
        raise MultimodalGoldenLiveInputError(
            "Live golden case IDs cannot be empty."
        )
    if len(set(normalized)) != len(normalized):
        raise MultimodalGoldenLiveInputError(
            "Live golden case IDs must be unique."
        )
    available = {
        case["case_id"]: case
    ...
    if unknown:
        raise MultimodalGoldenLiveInputError(
            "Unknown live golden case IDs: "
            + ", ".join(unknown)
        )
    return [available[case_id] for case_id in normalized]


def provider_call_plan(
        ...
        rows.append(
            {
                ...
                "maximum_calls": (
                    semantic_calls + observation_calls
                ),
            }
        )
    ...
    return {
        ...
        "exact_pre_run_count_reason": (
            "Each source pair always needs two semantic calls; "
            "the two claim-observation calls happen only when "
            "that pair survives the model-dependent exact-claim gate."
        ),
        "cases": rows,
    }
...
def labels_from_members(
    ...
    *,
    ...
    for raw in members:
        ...
        label = clean(
            id_to_label.get(
                clean(raw.get("capture_record_id"))
            )
        )
...
def score_case(
    *,
    ...
    expected_status = (
        clean(full.get("expected_status"))
        or "completed"
    )
    failures = []
    ...
    if expected_status == "completed":
        accepted = set(
            observed.get("accepted_member_labels")
            or []
        )
        rejected = set(
            observed.get("rejected_member_labels")
            or []
        )

        if not set(
            full["required_accept_labels"]
        ).issubset(accepted):
            failures.append("accepted_member_recall")

        if not set(
            full["required_reject_labels"]
        ).issubset(rejected):
            failures.append("rejected_member_recall")

        if int(observed.get("story_count") or 0) != int(
            full["expected_story_count"]
        ):
            failures.append("story_count")

        if clean(
            observed.get("merit_baseline_mode")
        ) != clean(
    ...
    *,
    ...
    observed = {
        ...
        "accepted_member_labels": labels_from_members(
            cluster.get("completed_members"),
            id_to_label=id_to_label,
        ),
        "rejected_member_labels": labels_from_members(
            cluster.get("rejected_members"),
            id_to_label=id_to_label,
        ),
        "story_count": int(graph.get("story_count") or 0),
        "story_ids": list(
            graph.get("story_ids", []) or []
        ),
        "claim_ids": list(
            cluster.get("claim_ids", []) or []
        ),
        "merit_baseline_mode": clean(
            (cluster.get("policy") or {}).get(
                "merit_baseline_mode"
            )
        ),
        "synthetic_merit_baseline_used": (
        ...
        "affects_live_merit": (
            "affects_live_merit" in true_flags
        ),
        "score_effect_applied": (
            "score_effect_applied" in true_flags
        ),
        "establishes_truth": (
            "establishes_truth" in true_flags
        ),
        "establishes_authority": (
            "establishes_authority" in true_flags
        ),
        "establishes_independence": (
            "establishes_independence" in true_flags
        ),
        "live_enablement_authorized": (
            "live_enablement_authorized"
            in true_flags
        ...
        "independence_inferred_from_identical_content": (
            identical
            and "establishes_independence"
            in true_flags
        ),
    }

    observed["score"] = score_case(
        case=case,
        observed=observed,
    )
    return observed
