...
class _StrictAutoShadowModel(BaseModel):
    class Config:
        extra = "forbid"


class MultimodalInboxAutoShadowRequest(
    _StrictAutoShadowModel
):
...
class MultimodalInboxAutoShadowResponse(
    BaseModel
):
    version: str
...
def build_router(
    *,
    ...
    @router.post(
        ...
        response_model=(
            MultimodalInboxAutoShadowResponse
        ),
    )
    def admin_multimodal_inbox_auto_shadow(
        req: MultimodalInboxAutoShadowRequest,
        request: Request,
    ):
        if not enabled:
            raise HTTPException(
        ...
        try:
            ...
            inbox_auto_shadow_orchestration
            .MultimodalInboxAutoShadowInputError
        ) as error:
            raise HTTPException(
                status_code=422,
                detail=str(error),
            ...
            inbox_auto_shadow_orchestration
            .MultimodalInboxAutoShadowDiscoveryError
        ) as error:
            raise HTTPException(
                status_code=409,
                detail=str(error),
            ...
            inbox_auto_shadow_orchestration
            .MultimodalInboxAutoShadowSelectionError
        ) as error:
            raise HTTPException(
                status_code=409,
                detail=str(error),
            ...
            inbox_auto_shadow_orchestration
            .MultimodalInboxAutoShadowProviderUnavailable
        ) as error:
            raise HTTPException(
                status_code=503,
                detail=str(error),
            ...
            inbox_auto_shadow_orchestration
            .MultimodalInboxAutoShadowExecutionError
        ) as error:
            raise HTTPException(
                status_code=409,
                detail=str(error),
            ...
            inbox_auto_shadow_orchestration
            .MultimodalInboxAutoShadowIntegrityError
        ) as error:
            raise HTTPException(
                status_code=500,
                detail=(
                    "Multimodal inbox automatic "
                    "shadow integrity validation failed."
                ),
            ) from error

        return MultimodalInboxAutoShadowResponse(
            **result
        )
