...
class EvidenceContextReporterRetrievalTests(
    unittest.TestCase
):
    def setUp(self):
        ...
        main.DB_PATH = (
            ...
            / "reporter-context-retrieval-test.db"
        )
    ...
    def test_exact_reporter_only_is_loaded(
        self,
    ):
        ...
        main.record_reporter_observation(
            reporter_id=self.other_reporter["id"],
            source_id=self.source["id"],
            subject_key=(
        ...
        evidence_match = main.record_evidence(
            ...
            reference_key="REPORTER-A-EVIDENCE",
        ...
        evidence_other = main.record_evidence(
            ...
            reference_key="REPORTER-B-EVIDENCE",
            verification_status="verified",
            observed_at=(
        ...
        context = (
            main.load_evidence_context_for_reporter(
                reporter_id=self.reporter["id"],
            )
        )

        self.assertEqual(
            context["scope"]["reporter_id"],
            self.reporter["id"],
        )
    ...
    def test_related_target_evidence_is_not_implicitly_loaded(
        ...
        media = main.upsert_media_item(
            ...
            content_hash="reporter-media-content",
            source_id=self.source["id"],
            reporter_id=self.reporter["id"],
            seen_at=(
                "2026-08-12T10:00:00+00:00"
        ...
        source_evidence = main.record_evidence(
            ...
            reference_key="SOURCE-TARGET-EVIDENCE",
            verification_status="verified",
            observed_at=(
                "2026-08-12T11:00:00+00:00"
            ),
    ...
    def test_empty_reporter_context_is_stable(
        self,
    ):
        first = (
            main.load_evidence_context_for_reporter(
        ...
        self.assertEqual(
            first["scope"]["reporter_id"],
            self.reporter["id"],
        )

    def test_reporter_id_is_required(
        self,
    ):
