...
export function AccountGate({ children }: PropsWithChildren) {
  ...
  return <View style={{ gap: 16 }}><ProductStatus loading={!account.ready} title="Your Sportabase account" detail={detail} />{account.ready ? <View style={{ alignItems: 'flex-start', flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}><ProductButton label={account.signedIn ? 'Retry account connection' : 'Sign in'} onPress={() => { setError(''); void (account.signedIn ? account.refresh() : account.signIn(false, '/settings')).catch((problem) => setError(problem instanceof Error ? problem.message : 'Account access is unavailable.')); }} variant="primary" />{!account.signedIn ? <ProductButton label="Create account" onPress={() => void account.signIn(true, '/settings').catch((problem) => setError(problem instanceof Error ? problem.message : 'Account creation is unavailable.'))} /> : null}</View> : null}{error || account.error ? <ProductStatus title="Account access is unavailable" detail={error || account.error} tone="error" /> : null}</View>;
}
