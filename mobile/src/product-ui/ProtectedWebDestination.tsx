...
type Props = PropsWithChildren<{ destination: Destination; returnDestination?: AuthReturnDestination; title: string }>;
...
export function ProtectedWebDestination({ children, destination, returnDestination = destination, title }: Props) {
  ...
  const beginSignIn = useCallback(async (signup = false) => {
    ...
    try { await account.signIn(signup, returnDestination); }
    catch (problem) {
  ...
  }, [account, destination, returnDestination, title]);
  ...
  return <ProductPage width="reading" testID="protected-account-gate">
