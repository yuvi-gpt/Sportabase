...
import type { DesignLabPalette } from './design-lab-theme';
...
function VerificationLane({ index, width, spec, mobile, debug, exit }: { index: number; width: number; spec: typeof laneSpecs[number]; mobile: boolean; debug: boolean; exit: Animated.Value }) {
  const { palette } = useDesignLabTheme();
  ...
  return <Animated.View style={[styles.lane, { width: laneWidth, opacity: exitOpacity }]}><View style={styles.rail} />
    ...
    <Animated.View style={[styles.signal, { width: signalWidth, opacity: signalOpacity, transform: [{ translateX: x }] }]}><LinearGradient colors={['transparent', palette.teal, 'transparent']} start={{ x: 0, y: .5 }} end={{ x: 1, y: .5 }} style={StyleSheet.absoluteFill} /></Animated.View>
  </Animated.View>;
}
...
const createStyles = (palette: DesignLabPalette) => StyleSheet.create({
  ...
  lane: { position: 'relative', height: 10, justifyContent: 'center', alignSelf: 'flex-end' }, rail: { height: 1, backgroundColor: palette.line }, tick: { position: 'absolute', top: 2, width: 1, height: 6, backgroundColor: palette.green, opacity: .22 }, signal: { position: 'absolute', top: 4, height: 2 },
  axis: { position: 'absolute', top: 22, bottom: 42, width: 1, backgroundColor: palette.line, opacity: .7 }, axisActive: { position: 'absolute', top: '28%', width: 1, height: '28%', backgroundColor: palette.teal },
  resolveLine: { position: 'absolute', right: 0, bottom: 4, flexDirection: 'row', alignItems: 'center', gap: 10 }, resolveLabel: { color: palette.muted, opacity: .48, fontSize: 9, fontWeight: '800', letterSpacing: 1.5 }, resolveMark: { width: 28, height: 1, backgroundColor: palette.lime },
});
